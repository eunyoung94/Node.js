...
var mysql=require("mysql");//외부모듈 , npm install 설치해야한다.
var con;//접속정보를 가진객체 ( 이객체를 이용하여 쿼리문을 수행할 수 있다. )
    ...
    if(result.pathname=="/login"){
        ...
        con.query(sql,function(error,record,fields){
            if(error){
                ...
                if(record.length>0){
                    //레코드가 있을때는(배열의 길이가1), 로그인 성공 메세지
                    // console.log("인증성공");//클라이언트 브라우저에 출력!
                    response.end("<script>alert('인증성공');history.back();</script>");
                }else{
                    //레코드가 없을때는(배열의 길이가0), 로그인 실패 메세지
                    response.end("<script>alert('인증성공');history.back();</script>");
                }
            }
